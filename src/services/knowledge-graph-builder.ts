...
export class KnowledgeGraphBuilderImpl implements KnowledgeGraphBuilder {
   ...
   */
  async saveCorrections(programName: string, corrections: any[], correctedText: string): Promise<void> {
    this.ensureConnected();
    this.logger.info('KnowledgeGraph', `Saving corrections for program: ${programName}`);
    return this.executeWrite(async (session) => {
      await session.run(`
