...
El pipeline utiliza el **Agent Development Kit (ADK)** de Google para orquestar secuencialmente cinco agentes especialistas, gestionados a través de `SequentialAgent` e `InMemoryRunner` en [multi-agent-service.ts](file:///media/dracero/08c67654-6ed7-4725-b74e-50f29ea60cb21/pythonAI-Others/grafo-test/src/services/multi-agent-service.ts).
...
graph TD
    ...
    D --> E(ComplianceValidatorAgent)
    E --> F(ProgramFixerAgent)
    F --> G[Informe de Adecuación]
    G --> H[Generador PDFKit]
    H --> I[PDF de Adecuación Curricular]
```
...
*   **Propósito**: Cruzar la ontología normativa y el programa actual para consolidar las brechas de cumplimiento iniciales (requisitos faltantes o parcialmente cubiertos).
...
### D. `ComplianceValidatorAgent` (Agente Validador de Cumplimiento Semántico)
*   **Propósito**: Analizar semánticamente las brechas de cumplimiento detectadas y el texto original para descartar falsos positivos y regularidades negativas legítimas.
*   **Directivas de Exclusión**:
    1.  **Declaraciones Negativas Legítimas**: Si un requisito exige detallar o regular un aspecto y el programa indica explícitamente que no aplica o que ninguna actividad está sujeta a ello, el agente lo marca como falso positivo y lo descarta (cumplimiento semántico).
    2.  **No Aplicabilidad por Naturaleza**: Excluir requisitos de equipamiento o infraestructura inaplicables por la naturaleza de la materia.
*   **Salida**: Un JSON depurado conteniendo las brechas reales validadas (`validatedGaps`) y falsos positivos explicados (`excludedGaps`).

### E. `ProgramFixerAgent` (Agente Redactor de Adecuaciones)
*   **Propósito**: Consolidar los resultados previos en un informe de adecuación final que cubra únicamente las brechas reales y validadas.
*   **Formato de Salida**: Genera un informe estructurado de correcciones aplicables directamente sobre el documento original.
